...
async function voteCard(cardId, vote, pillEl) {
  ...
  try {
    ...
    // ✅ OPTIONAL (safe): if we already know we voted today, don’t prompt Phantom signing
    ...
    // ✅ IMPORTANT: use HTTP 429 (server source of truth) instead of guessing strings
    if (res.status === 429) {
    ...
    // ✅ keep your existing local mark (this is good)
    markVotedToday(publicKeyBase58, cardId);
    ...
    // Keep your existing nicer mapping, but it’s now a fallback (429 handled above)
    const nicer =
